...
export class ArrayParser<T> extends AdvancedParser<T[]> {
    ...
    readConfigNumber(ctx: ParserContext, config: ArrayParserOptionNumber, option?: Partial<AccessOption>): number {
    ...
    writeConfigNumber(ctx: ParserContext, config: ArrayParserOptionNumber, value: number, option?: Partial<AccessOption>): number {
    ...
    read(ctx: ParserContext): T[] {
        ...
        if (!isUndefined(sizeOption)) {
            ...
            while (true) {
                const collectedSize = ctx.pos - pos;
                if (collectedSize > sizeValue) throw Error('Invalid array data read');
                if (collectedSize === sizeValue) break;
                ctx.expose($index, items.length);
                ctx.expose(ctx.constant.$path, `${ parentPath }[${ items.length }]`);
                items.push(ctx.read(itemParser));
            }
    ...
    write(ctx: ParserContext, items: T[]) {
        ...
        if (!isUndefined(sizeOption)) {
            ...
            // 回到初始位置，写入正确的size
            const afterPos = ctx.pos;
            this.writeConfigNumber(ctx, sizeOption, afterPos - beforePos, { pos: beforePos, consume: false });
            return;
        }
        ...
        if (!isUndefined(endsOption)) {
            ...
            for (const [ idx, item ] of items.entries()) {
                ...
                ctx.expose($index, idx);
                ctx.write(itemParser, item);
                const startByte = ctx.read(Uint8, { pos, consume: false });
