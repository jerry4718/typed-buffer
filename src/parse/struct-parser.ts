...
import { ContextCompute, AccessOption, ParserContext } from '../context/types.ts';
...
export class StructParser<T extends object> extends AdvancedParser<T> {
    ...
    private readonly option?: AccessOption;
    ...
    resolveOption(ctx: ParserContext, fieldConfig: StructFieldActual<T, keyof T>): Partial<AccessOption> | undefined {
        const inputPoint = (fieldConfig as StructFieldActual<T, keyof T>).point;
        if (isUndefined(inputPoint)) return this.option;

        if (isNumber(inputPoint)) return { ...this.option, pos: inputPoint };
        if (isFunction(inputPoint)) return { ...this.option, pos: ctx.compute(inputPoint)! };
    ...
    resolveIf(ctx: ParserContext, fieldConfig: StructFieldActual<T, keyof T>): boolean {
        ...
        if (isUndefined(fieldIf)) return true;
    ...
    resolveDefault(ctx: ParserContext, fieldConfig: StructFieldActual<T, keyof T>): T[keyof T] | undefined {
        ...
        if (isUndefined(fieldDefault)) return undefined;
    ...
    applyExpose(ctx: ParserContext, fieldConfig: StructField<T, keyof T>, value: T[keyof T]) {
        ...
        if (isUndefined(fieldExpose)) return;

        if (isBoolean(fieldExpose)) return ctx.expose(fieldConfig.name, value);
        if (isString(fieldExpose) || isSymbol(fieldExpose)) return ctx.expose(fieldExpose, value);
    }
