...
import { ContextCompute, ContextConstant, AccessOption } from '../context/types.ts';
...
export function StructTarget<T extends object>(config: Partial<ContextConstant & AccessOption> = {}) {
    function decorator<Class extends Constructor<T>>(klass: Class) {
        Reflect.defineMetadata(kParserTarget, config, klass);

        const parser = convertStructParser(klass);
...
function convertStructParser<T extends object>(klass: Constructor<T>): StructParser<T> {
...
function createFieldTypeDecorator<T>(parser: FieldConfig<string | symbol, T>['type']) {
    return definePropertyDecorator(function <K extends string | symbol>(proto: object, propertyKey: K) {
        const fieldConfig = ensureFieldConfig<FieldConfig<K, T>>(kParserFields, proto, propertyKey);
